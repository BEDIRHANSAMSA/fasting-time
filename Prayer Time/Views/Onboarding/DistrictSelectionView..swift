...
struct DistrictSelectionView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 6) {
                ...
                Text("Son olarak, \(city.name.titleCase) ilinde bulunduğun ilçeyi seç. Eğer bulunduğun ilçe gözükmüyorsa aynı ili seçebilirsin.")
                    .font(.body)
    ...
    private func selectDistrict(_ district: Location.District) {
        ...
        Task {
            await prayerTimesManager.fetchPrayerTimes(for: district.id)
        }
    }
}
